...
import { Button, Col, FloatingLabel, FormControl, FormSelect, Modal, ModalBody, ModalFooter, ModalHeader, Row } from 'react-bootstrap';
import useProductContext from '../hooks/useProductContext';

const EditModal = () =>
{
  const {description,setDescription,setSrc,price,setPrice,category,setCategory,quantity,setQuantity,name,setName,handleEdit,editModal, id,handleEditClose} = useProductContext()
  const onImgChange = ( e ) =>
  {
  ...
  return (
    <Modal show={ editModal } centered scrollable onHide={handleEditClose} >
      <ModalHeader closeButton>
        <p className="h5 text-center text-capitalize">Edit Product</p>
      </ModalHeader>
      <ModalBody>
        <Row>
      ...
      <ModalFooter>
        <Col xs={ 8 } md={ 6 } className='mx-auto text-center mt-3'>
          <Button variant='success' onClick={()=>handleEdit(id)} >Save</Button>
        </Col>
      </ModalFooter>
    </Modal>
  )
